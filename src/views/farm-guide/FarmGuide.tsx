  ...
  return (
    <div className='farm'>
      ...
      <div className='section common'>
        <Card>
          <CardBody className='py-2' style={{display: 'grid', gridTemplateRows: 'auto 1fr', columnGap: '0.75rem', justifyItems: 'center'}}>
            <h3 className='text-center'>Stats</h3>
            <div className='d-flex flex-column align-items-center'>
              <div><strong className='text-right'>{farm.bps.toFixed(2)}</strong> BPS</div>
        ...
        <Card>
          <CardBody className='py-2' style={{display: 'grid', gridTemplateRows: 'auto 1fr', columnGap: '0.75rem', justifyItems: 'center'}}>
            ...
            <div>
              <a href={farm.schema} download className='d-flex align-items-end gap-2'><Icon icon={ICONS.Download} size={ICON_SIZE.SIZE_32} /> Download</a>
            </div>
          </CardBody>
        </Card>
        <Card>
          <CardBody className='py-2' style={{display: 'grid', gridTemplateRows: 'auto 1fr', columnGap: '0.75rem', justifyItems: 'center'}}>
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'auto 1fr', columnGap: '0.75rem', height: 'fit-content'}}>
        ...
        <Card>
          <CardBody className='py-2'>
            ...
            {
              farm.enchantments.map((enchantment) => <div key={enchantment}><strong>{enchantment}</strong></div>)
            }
            </div>
          </CardBody>
        </Card>
        <Card>
          <CardBody className='py-2'>
            ...
            <div className='keyboard-layout'>
              {farm.keys.includes(Keys.W) ? <Icon icon={ICONS.KeyWPressed} size={ICON_SIZE.SIZE_FIT}/> :
                <Icon icon={ICONS.KeyW} size={ICON_SIZE.SIZE_FIT}/>}
              {farm.keys.includes(Keys.A) ? <Icon icon={ICONS.KeyAPressed} size={ICON_SIZE.SIZE_FIT}/> :
                <Icon icon={ICONS.KeyA} size={ICON_SIZE.SIZE_FIT}/>}
              {farm.keys.includes(Keys.S) ? <Icon icon={ICONS.KeySPressed} size={ICON_SIZE.SIZE_FIT}/> :
                <Icon icon={ICONS.KeyS} size={ICON_SIZE.SIZE_FIT}/>}
              {farm.keys.includes(Keys.D) ? <Icon icon={ICONS.KeyDPressed} size={ICON_SIZE.SIZE_FIT}/> :
